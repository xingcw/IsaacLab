...
class QuadcopterEnv(DirectRLEnv):
    ...
    # def _get_rewards(self) -> torch.Tensor:
    #     dist   = torch.norm(self._desired_pos_w - self._robot.data.root_link_pos_w, dim=1)
    #     speed  = torch.norm(self._robot.data.root_com_lin_vel_b, dim=1)
    #     smooth = torch.sum((self._actions - self._previous_action)**2, dim=1)
    ...
    #     goal_dir = self._desired_pos_w - self._robot.data.root_link_pos_w
    #     goal_dir = torch.nn.functional.normalize(goal_dir, dim=1)
    #     velocity = self._robot.data.root_com_lin_vel_b
    #     align_reward = 0.5 * torch.sum(velocity * goal_dir, dim=1)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # cond_h_min_time = episode_time > self.cfg.max_time_on_ground
        ...
        died = cond_h_min_time | cond_not_converged
